...
- Freeze constructor-derived hot-path state on formatter instances. Cached method calls must not redo locale negotiation, option validation, digit-option resolution, plural-rule lookup, interval-pattern selection, or embedded formatter construction. DateTimeFormat compiles its semantic range record and distinguishing cross-date fallbacks in `New`; DurationFormat composes constructor-resolved `NumberFormat` and `ListFormat` instances.
