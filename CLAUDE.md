...
- `tools/conformance.RunFixtures` must load and validate one immutable suite before any fixture callback runs. Malformed, expired, unknown, or mismatched divergence/XFAIL records fail the package test; do not add best-effort per-fixture skip lookup that hides ledger errors.
...
Nested tool modules are checked from their own module roots. Do not use root-module patterns such as `go test ./tools/gen-cldr/...`; run from each module root instead, for example `(cd tools/gen-cldr && go test ./...)`, `(cd tools/gen-plural-rules && go test ./...)`, and `(cd tools/gen-fixtures-from-formatjs && go test ./...)`. `task data:check` is the standing generated-data gate and includes `tools/gen-cldr` test/vet; the direct commands remain useful for focused development.
