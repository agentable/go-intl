...
- [ ] The codegen tool currently reads `ca-gregorian.json`, the output `datesByLocale` active scope only contains the `"gregorian"` calendar, and `SupportedCalendars()` derives `gregory` / `iso8601` from that generated payload rather than a hand-written list.
