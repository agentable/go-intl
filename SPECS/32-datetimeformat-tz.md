...
3. The table storage path **MUST** be `internal/cldr/timezones.go`(generated).
4. Form:
   ```go
...
1. The three-segment mapping **MUST** be output by codegen as Go literal, and the generated location is `internal/cldr/metazones.go`.
...
// Stage 3: locale × zone → exemplar city
   ...
   ```
4. accessor `internal/cldr.TimeZoneDisplayName(loc cldr.Locale, zone string, form TimeZoneName, isDST bool, instant int64, offsetMs int64) string` **MUST** implement ECMA-402 §13.1.5 fallback chain:
   ```text
...
### 2.1 active scope scope

**MUST** Rules:

1. active scope calendar pattern data **MUST** only generate Gregorian. `SupportedCalendars()` must return exactly `gregory` and ECMA-402 required `iso8601`; other well-formed calendar requests returning errors matching `gointl.ErrUnsupportedOption` during construction must not silently fall back to Gregorian behavior.
2. The codegen tool (`tools/gen-cldr/`) **MUST** explicitly skip non-Gregorian pattern payload; if it is forced to generate unconsumed payload, build break.
3. Calendar data storage path **MUST** be `internal/cldr/dates.go`(generated).
...
> **Why**:
> 1. SPEC 30 §3 has declared active scope Gregorian only.
...
Gregorian data schema in `internal/cldr/dates.go` must be aligned to CLDR `cldr-dates-full/main/<locale>/ca-gregorian.json`:
...
3. The accessor `internal/cldr.GregorianFor(loc cldr.Locale) Gregorian` **MUST** receive the parsed CLDR locale handle; the matching from public `locale.Locale` to `cldr.Locale` is completed by locale negotiation + `internal/cldr.ResolveLocale(tag string)` during the formatter construction period.
...
1. `dayPeriodRules` **MUST** be fully embedded into `internal/cldr/dates.go` to generate Go literal:
   ```go
// internal/cldr/dates.go (fragment)
...
From, To time.Duration // Time offset from 00:00 on the current day
   ...
   ```
2. `internal/cldr.DayPeriodFor(loc cldr.Locale, hour, minute int) string` **MUST** return `"morning1"` / `"noon"` and other type names; the caller then checks `Gregorian.DayPeriods.Flex[type]`.
...
2. `internal/cldr.GregorianFor(loc)` **MUST** be based on the parsed `cldr.Locale` constant level lookup table, and the BCP 47 string must not be parsed during the call period.
...
- **FORBIDDEN** active scope implements any calendar data (including Buddhist placeholders) other than Gregorian.
...
- [ ] After `internal/cldr.ResolveLocale("en-US")` succeeds, `internal/cldr.GregorianFor(loc)` returns the complete `Gregorian` structure, all fields are non-null.
- [ ] After `internal/cldr.ResolveLocale("zh-Hans-CN")` succeeds, `internal/cldr.GregorianFor(loc).DayPeriods.Flex` contains all 6 keys of `"morning1" / "afternoon1" / "evening1" / "night1" / "noon" / "midnight"`.
...
- [ ] `internal/cldr/metazones.go` is generated, and the file header contains `// Code generated by tools/gen-cldr; DO NOT EDIT.`.
- [ ] `internal/cldr/dates.go` is generated, the file header contains `// Code generated by tools/gen-cldr; DO NOT EDIT.`; there is no `internal/cldr/calendars/*` active scope placeholder file.
- [ ] The codegen tool only reads `ca-gregorian.json`, and the output `datesByLocale` active scope only contains the `"gregorian"` calendar.
