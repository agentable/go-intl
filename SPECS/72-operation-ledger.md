...
| Date ranges | `formatRange`, `formatRangeToParts`, including `fractionalSecond` range records | `datetimeformat/range.go`, shared compiled endpoint/source steps, `RangePart`, `PartFractionalSecond` | `datetimeformat/range_execution_test.go`, `datetimeformat/datetimeformat_test.go`, `record_json_test.go`, invalid-range tests, conformance fixtures |
