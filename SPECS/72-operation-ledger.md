...
| Root supported values | `Intl.supportedValuesOf` | `supported.go`, typed supported-value accessors; time zones project `internal/tz` primary records | `supported_test.go`, generated CLDR/IANA/tz data contract |
| Root constructor aliases | `Intl.<Constructor>` namespace properties | `intl.go` type aliases | `intl_test.go`, SPEC 60 |
| Structured error details | Go error bridge for ECMA-402 `RangeError` / `TypeError`-equivalent failures | `errors.go`, `internal/intlerr/errors.go`, `internal/ecma402/errors.go`, `ErrorKind`, `Error`, four reachable root category sentinels | `internal/intlerr/errors_test.go`, `internal/ecma402/errors_test.go`, constructor option tests, public error-text tests |
| ECMA-402 record JSON | Host-boundary bridge for `resolvedOptions()`, parts, range parts, locale info, and duration records | package `ResolvedOptions` structs, package `Part` / `RangePart` structs, `locale.WeekInfo`, `locale.TextInfo`, `durationformat.Duration` | `record_json_test.go` |
...
| Locale string identity | `Intl.Locale.prototype.toString`, `baseName` | `Locale.String`, suffix-free `BaseName`, `MarshalText`, `UnmarshalText` | `locale/locale_test.go`, `locale/extension_test.go` |
...
| Locale maximize/minimize | `maximize`, `minimize` | `locale/canonical.go`, language/script/region replacement preserving variants and every extension | `locale/canonical_test.go`, CLDR likely-subtag tests |
...
| Plural selection | `Intl.PluralRules.prototype.select` | `pluralrules/pluralrules.go`, constructor-resolved generated rule, `Value` constructors, total `Select(Value) Category` | `pluralrules/pluralrules_test.go`, `internal/cldr/plural/rules_test.go`, conformance fixtures |
| Plural range selection | `selectRange` | `pluralrules/range.go`, `SelectRange` | `pluralrules/range_test.go`, conformance fixtures |
...
| Plural resolved-option presence fields | `Intl.PluralRules.prototype.resolvedOptions()` non-undefined digit slots and branch properties | shared digit-slot projection into `ResolvedOptions` pointer fields plus `CompactDisplay`; more/less precision exposes both fraction and significant families | `pluralrules/options_test.go`, `record_json_test.go`, conformance fixtures |
...
| Date parts | `formatToParts`, including `fractionalSecond` records | `datetimeformat/parts.go`, `Part`, `PartFractionalSecond` | `datetimeformat/datetimeformat_test.go`, `record_json_test.go`, conformance fixtures |
| Date ranges | `formatRange`, `formatRangeToParts`, including `fractionalSecond` range records | constructor-compiled `datetimeformat/range_record.go`, semantic localized-field relation, distinguishing cross-date fallback, shared endpoint/source steps in `range.go`, `RangePart`, `PartFractionalSecond` | `datetimeformat/range_relation_test.go`, `datetimeformat/range_execution_test.go`, `datetimeformat/datetimeformat_test.go`, `record_json_test.go`, manual/generated conformance fixtures |
...
| Relative time formatting | `Intl.RelativeTimeFormat.prototype.format` | `relativetimeformat/format.go`, single ECMAScript Number `Value` bridge + `Format`; `Int`/`Uint` convert through `float64`; `LocaleMatcher *string`, `NumberingSystem *string`, `Style *string`, and `Numeric *string` option presence bridges | `relativetimeformat/number_semantics_test.go`, `relativetimeformat/relativetimeformat_test.go`, conformance fixtures |
| Relative time parts | `formatToParts` | same normalized Number `Value` bridge + `FormatToParts` | `relativetimeformat/number_semantics_test.go`, `relativetimeformat/parts_test.go`, conformance fixtures |
| Duration formatting | `Intl.DurationFormat.prototype.format`, `ToDurationRecord`, `ToIntegerIfIntegral` | `durationformat.Duration` `float64` Number fields, one private exact-integer projection, `Format` | `durationformat/number_boundary_test.go`, `durationformat/durationformat_test.go`, conformance fixtures |
| Duration parts | `formatToParts` | the same projected duration record + `FormatToParts` | `durationformat/number_boundary_test.go`, `durationformat/parts_test.go`, conformance fixtures |
