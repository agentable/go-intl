...
func (f *DateTimeFormat) fallbackRangeParts(start, end []Part) []RangePart {
	pattern := f.gregorian.IntervalFallback
	if pattern == "" {
		pattern = "{0} – {1}"
	}
	...
	for pattern != "" {
		...
		if idx < 0 {
			return appendRangeLiteralPart(parts, pattern, SourceShared)
		}
		if idx > 0 {
			parts = appendRangeLiteralPart(parts, pattern[:idx], SourceShared)
			pattern = pattern[idx:]
			continue
		}
