...
func TestTimeZoneDisplayNameZoneSpecificName(t *testing.T) {
	t.Parallel()

	loc, ok := ResolveLocale("en-GB")
	if !ok {
		t.Fatal("ResolveLocale(en-GB) ok=false")
	}
	instant := time.Date(2021, time.June, 10, 12, 0, 0, 0, time.UTC).UnixMilli()
	if got, want := TimeZoneDisplayName(loc, "Europe/London", TimeZoneNameLong, true, instant, 3600*1000), "British Summer Time"; got != want {
		t.Fatalf("TimeZoneDisplayName(Europe/London daylight) = %q, want %q", got, want)
	}
}
