...
func SupportedLocalesOf(locales []locale.Locale, opts ...Options) ([]locale.Locale, error) {
	...
	return localematcher.FilterLocales(plural.SupportedLocales(), locales, matcher), nil
}

func supportedLocaleMatcher(opts ...Options) (localematcher.Algorithm, error) {
	if len(opts) > 1 {
		return 0, fmt.Errorf("pluralrules: multiple options: %w", ErrInvalidOption)
	}
	...
	return 0, invalidOption("localeMatcher", string(opts[0].LocaleMatcher))
}
