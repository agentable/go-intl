...
func loadMetazones(root string, locales []string) (Metazones, error) {
	...
	names, cities, formats, err := loadTimeZoneNames(root, locales)
	if err != nil {
		return Metazones{}, err
	}
	return Metazones{ZoneToMetazones: zones, Names: names, ExemplarCities: cities, Formats: formats}, nil
}
...
func loadTimeZoneNames(root string, locales []string) (map[string]map[string]MetazoneNames, map[string]map[string]string, map[string]TimeZoneFormats, error) {
	...
	for _, locale := range locales {
		...
		if !ok {
			return nil, nil, nil, fmt.Errorf("timeZoneNames body missing for %s", locale)
		}
		...
		for area, byZone := range body.Dates.TimeZoneNames.Zone {
			for zone, value := range byZone {
				if value.ExemplarCity != "" {
					localeCities[area+"/"+zone] = value.ExemplarCity
				}
			}
		}
	...
	return names, cities, formats, nil
}
