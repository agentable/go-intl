...
func TestRunGeneratesLocalesAndLikelySubtags(t *testing.T) {
	...
	if !containsAll(string(likely), "MaximizeSubtags", "MinimizeSubtags", "zh_Hans_CN") {
...
func TestRunPrefersLanguageMinimizeAlias(t *testing.T) {
	...
	if !strings.Contains(string(generated), `}: "zh",`) || strings.Contains(string(generated), `}: "und-CN"`) {
...
func writePhase3CLDRFixture(t *testing.T, root string) {
	t.Helper()
	...
	if err := os.WriteFile(filepath.Join(root, "cldr-core", "package.json"), []byte(`{"name":"cldr-core","version":"48.1.0"}`), 0o666); err != nil {
...
func writeLocaleProfileFixture(t *testing.T, dir string) string {
	...
	profile := `{"locales":["en","en-US","zh","zh-Hans","zh-Hans-CN"],"formatterLocales":["en","en-US","zh","zh-Hans","zh-Hans-CN"]}`
