...
func importFormatJSNumberFormat(path, outDir string) ([]skipEntry, error) {
	...
	if err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		case ".snap":
			skips = append(skips, skipEntry{Source: formatJSNumberFormatTestSourcePrefix + rel, Category: "snapshot-source", Reason: "snapshot output requires source test input mapping"})
			return nil
		case ".ts":
			data, err := os.ReadFile(path)
			if err != nil {
...
func importFormatJSPluralRules(path, outDir string) ([]skipEntry, error) {
	...
	if err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		switch filepath.Ext(path) {
		case ".snap":
			skips = append(skips, skipEntry{Source: formatJSPluralRulesTestSourcePrefix + rel, Category: "snapshot-source", Reason: "snapshot output requires source test input mapping"})
			return nil
		case ".ts":
			data, err := os.ReadFile(path)
			if err != nil {
...
func importFormatJSDateTimeFormat(path, outDir string) ([]skipEntry, error) {
	...
	if err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		switch filepath.Ext(path) {
		case ".snap":
			skips = append(skips, skipEntry{Source: formatJSDateTimeFormatTestSourcePrefix + rel, Category: "snapshot-source", Reason: "snapshot output requires source test input mapping"})
			return nil
		case ".ts":
			data, err := os.ReadFile(path)
			if err != nil {
...
func importFormatJSLocaleFixtures(root, targetRoot, sourcePrefix string, extract func(string, string) []fixture, slug func(string) string) ([]skipEntry, error) {
	...
	if err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		switch filepath.Ext(path) {
		case ".snap":
			skips = append(skips, skipEntry{Source: sourcePrefix + rel, Category: "snapshot-source", Reason: "snapshot output requires source test input mapping"})
			return nil
		case ".ts":
			data, err := os.ReadFile(path)
			if err != nil {
