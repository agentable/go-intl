...
func TestRunGeneratesPluralRuleFiles(t *testing.T) {
	...
	if !containsAll(string(cardinal), "func CardinalRule", "cardinalEnUS", "cardinalZhHansCN", "cardinalPl", "o.N.ModInt(100).BetweenInt(3, 10)") {
	...
	facade, err := os.ReadFile(filepath.Join(dir, "plurals.go"))
	if err != nil {
		t.Fatal(err)
	}
	if !containsAll(string(facade), "func (l Locale) Cardinal", "func (l Locale) Ordinal") {
		t.Fatalf("plurals.go missing facade accessors:\n%s", facade)
	}
}
...
func TestCompileCondition(t *testing.T) {
	...
	tests := []struct {
		...
		{in: "i = 1 and v = 0", want: "o.I.EqualInt(1) && o.V == 0"},
		{in: "i % 10 = 2..4 and i % 100 != 12..14", want: "o.I.ModInt(10).BetweenInt(2, 4) && o.I.ModInt(100).OutsideIntRange(12, 14)"},
		{in: "n % 100 = 3..10", want: "o.N.ModInt(100).BetweenInt(3, 10)"},
	}
...
func writeLocaleProfileFixture(t *testing.T, dir string) string {
	...
	profile := `{"locales":["ar","en","en-US","fr","pl","zh","zh-Hans","zh-Hans-CN"],"formatterLocales":["en","en-US","zh","zh-Hans","zh-Hans-CN"],"pluralLocales":["ar","en","en-US","fr","pl","zh","zh-Hans","zh-Hans-CN"]}`
